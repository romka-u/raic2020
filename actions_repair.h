    ...
    for (const auto& bu : world.myBuildings) {
        if (bu.health < bu.maxHealth) {
            for (const auto& wrk : world.myWorkers) {
                ...
                if (cd == 1) {
                    actions.emplace_back(wrk.id, A_REPAIR, NOWHERE, bu.id, Score(120 + 8 * (bu.entityType == EntityType::TURRET), 0));
                } else {
                    actions.emplace_back(wrk.id, A_REPAIR_MOVE, bu.position, -1, Score(101 - cd + 16 * (bu.entityType == EntityType::TURRET), 0));
                }
            }
        }
    }
